...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 bg-white z-50  ">
      <LayoutWrapper>
        {/* Show skeleton for the entire header when loading */}
        {loading ? (
          <div className="w-full h-16 flex items-center px-4 animate-pulse">
            <div className="h-10 w-40 bg-gray-200 rounded mr-8" /> {/* Logo skeleton */}
            <div className="flex-1 flex space-x-8">
              {[0, 1, 2, 3, 4].map((i) => (
                <div key={i} className="h-5 w-24 bg-gray-200 rounded" />
              ))}
            </div>
            <div className="h-10 w-10 bg-gray-200 rounded-full ml-8" /> {/* Avatar/menu skeleton */}
          </div>
        ) : (
          <>
            <nav className="flex justify-between items-center h-20">
              ...
              >
                <svg width="28" height="28" fill="none" viewBox="0 0 24 24">
                  <path stroke="#189BA3" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
              </button>
                    ...
                    >
                      ...
                      {loading
                        ...
                        : menuTypes.slice(0, 2).map((type, idx) => (
                            <Link
                              key={type.id}
                              href={`${NavItems[idx + 1].href}?typeid=${type.id}&name=${encodeURIComponent(type.name)}`}
                              className="capitalize text-sm font-[400] font-ubuntu relative group"
                            >
                              {type.name}
                              <span className="absolute -bottom-1 left-0 w-full h-1 bg-teal-500 transform scale-x-0 transition-transform duration-500 group-hover:scale-x-100" />
            ...
            {/* Restore search/filter bar below nav, only when not scrolled and not onboarding */}
            {pathName !== "/partner/onboarding" && (
              <div
