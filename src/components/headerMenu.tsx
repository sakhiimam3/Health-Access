...
const CustomDropdownMenu = () => {
  ...
  const isCustomer = partnerProfile?.data?.user?.role === "customer";
  const userImage = isCustomer ? partnerProfile?.data?.user?.image : partnerProfile?.data?.image;
  const userName = `${partnerProfile?.data?.user?.firstName || ""} ${partnerProfile?.data?.user?.lastName || ""}`.trim();
  const userEmail = partnerProfile?.data?.user?.email;
